    ...
    public function show(Http_Request $request)
    {
        if ($this->isLogged()) {
            ...
            foreach ($_SESSION['SELECTIONS'] as $selection) {
                $array_election = $this->getController()->getSelectionById($selection['id'])[0];
                $cn_models = $this->getController()->getCnModelBySelection($selection['id']);
                $array_election['cn_models'] = $cn_models;
                $array_election['selections']['cn_models'] = $cn_models;
                $selections[] = $array_election;
            }
