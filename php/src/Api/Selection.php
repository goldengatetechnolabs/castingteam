...
class Api_Selection extends Core_Controller
{
    ...
    public function add(Http_Request $request)
    {
        ...
        $selectionId = isset($post['selection_id']) ? $post['selection_id'] : '';
        $selectionId = trim($selectionId);
        $modelId = isset($post['model_id']) ? $post['model_id'] : '';
        $modelId = trim($modelId);
        $selectionTitle = isset($post['selection_title']) ? $post['selection_title'] : '';
        $selectionTitle = trim($selectionTitle);

        if ($this->isLogged()) {
            $userId = (new Core_SessionHandler())->get(SESSION_NAME_SITE_LOGIN);

            if (!empty($selectionId)) {
                if(!$this->controller->getModelsBySelectionIdAndModelId($selectionId, $modelId)) {
                    $this->controller->addModelToSelection($request->getPost());
                }
            } elseif (!empty($selectionTitle)) {
                $data = [
                ...
                if(!$this->controller->getModelsBySelectionIdAndModelId($data['selection_id'], $data['model_id'])) {
        ...
        return
            new Http_JsonResponse(
                [
                    ...
                    'message' => $message,
                    'data' => $response,
                    'count' => $this->getSelectionModelsCount(),
                ]
            );
    }
