...
use CastingteamBundle\Entity\Keyword;

use CastingteamBundle\Entity\Model;



class Registreren extends Core_Controller

{
...

    {
        ...
        Flight::smarty()->display('wachtwoord.html');

    }



    public function stap0()

    {
        ...
        Flight::smarty()->display('registration/stap0.tpl');
...

    {
        ...
        Flight::smarty()->assign('stap_passed', $_SESSION['registreren']['stap_passed']);

        Flight::smarty()->assign('stapActief', 1);

        Flight::smarty()->display('registration/stap1.tpl');
...

    {
        ...
        if (!isset($_SESSION['registreren']['stap1'])) {
            ...
            if (isset($_SESSION['registreren']['stap1']) && $_SESSION['registreren']['stap1'] == TRUE) {
                ...
                Flight::smarty()->assign('eigenschappen', getEigenschappen());
                ...
                Flight::smarty()->assign('stapActief', 2);

                Flight::smarty()->assign('countries', DBController::getInstance()->getCountries());

                Flight::smarty()->display('registration/stap2.tpl');
...

    {
        ...
        if (!isset($_SESSION['registreren']['stap2'])) {
            ...
            Flight::smarty()->assign('stapActief', 3);

            Flight::smarty()->display('registration/stap3.tpl');
...

    {
        ...
        if (!isset($_SESSION['registreren']['stap3'])) {
            ...
            Flight::smarty()->assign('stapActief', 4);

            Flight::smarty()->display('registration/stap4.tpl');
...

    {
        ...
        if (!isset($_SESSION['registreren']['stap4'])) {
            ...
            Flight::smarty()->assign('stapActief', 5);

            Flight::smarty()->display('registration/stap5.tpl');
...

    {
        ...
        if (isset($_SESSION['model_id']) && is_numeric($_SESSION['model_id'])) {
            ...
            Flight::smarty()->assign('code', $code['code']);

            Flight::smarty()->assign('stap_passed', $_SESSION['registreren']['stap_passed']);

            Flight::smarty()->assign('stapActief', 6);

            Flight::smarty()->display('registration/stap6.tpl');
    ...
    public function ajax_naar_stap_2()

    {
        ...
        $_SESSION['registreren']['adres'] = ucwords($_POST['adres']);

        $_SESSION['registreren']['huisnummer'] = $_POST['huisnummer'];

        $_SESSION['registreren']['postcode'] = $_POST['postcode'];

        $_SESSION['registreren']['gemeente'] = ucwords($_POST['gemeente']);
        ...
        $_SESSION['registreren']['keywords'] = $_POST['keywords'];
        ...
        if (isset($_SESSION['model_id']) && is_numeric($_SESSION['model_id'])) {
            ...
            $model = $this->entityFactory->model()->findOneById($_SESSION['model_id']);
            ...
            if (!empty($_SESSION['registreren']['keywords'])) {

                $keyword = (new Keyword())->setWord($_SESSION['registreren']['keywords'])->setModel($model);
    ...
    public function ajax_naar_stap_3()
    ...
    public function ajax_get_fotos()

    {
        ...
        if (isset($_SESSION['model_id']) && is_numeric($_SESSION['model_id'])) {
            ...
            $query = Flight::db()->query("
                ...
                WHERE a.id_model=" . $_SESSION['model_id'] . "
            ...
            while ($r = $query->fetch_array()) {
                ...
                if ($groupDate != $photoDate) {
                    ...
                    $html_old .= '<div style="margin-top:40px; margin-bottom:10px;" class="info_title"><p class="cursief"><strong>Aangeleverde - ' . $photoDate . '</strong></p></div><ul class="uploadimages" id="uploadimages">';
        ...
        if (isset($_SESSION['registreren']['fotos']) && count($_SESSION['registreren']['fotos']) > 0) {
            ...
            $array = $_SESSION['registreren']['fotos'];
            ...
            foreach ($array AS $id => $num) {
                ...
                $html .= '<li><div style="cursor:pointer; height:190px; width:190px;  overflow: hidden;"><img class="delete" style="position: absolute; width: 32px; height: 32px; margin: 10px; margin-left:150px;" src="/images/remove.png" alt=""/><img class="model_photo" style="height: auto; width: auto; min-height:190px; min-width:190px;" src="/_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $id . '_thumb.jpg?date=' . microtime() . '" alt="" /></div></li>';
        ...
        echo json_encode(['data' => ['new' => $html, 'old' => $html_old]]);
...

    {
        ...
        if (isset($_POST['img'])) {
            ...
            if (strpos($img, '/_tmp/') !== FALSE) {

                $id = str_replace('/_tmp/' . $_SESSION['registreren']['fotos_code'] . '/', '', $img);

                $id = str_replace('_thumb.jpg', '', $id);
            ...
            if (is_numeric($id)) {
                ...
                if (isset($_SESSION['registreren']['fotos'][$id]) && $soort == 'tmp') {

                    unset($_SESSION['registreren']['fotos'][$id]);



                    @unlink('_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $id . '_thumb.jpg');

                    @unlink('_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $id . '.jpg');



                    $oude_array = $_SESSION['registreren']['fotos'];
                    ...
                    foreach ($oude_array AS $key => $val) {

                        $nieuwe_array[$teller] = 1;

                        @rename('_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $key . '_thumb.jpg', '_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $teller . '_thumb.jpg');

                        @rename('_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $key . '.jpg', '_tmp/' . $_SESSION['registreren']['fotos_code'] . '/' . $teller . '.jpg');
                    ...
                    $_SESSION['registreren']['fotos'] = $nieuwe_array;
        ...
        if (isset($_SESSION['model_id']) && is_numeric($_SESSION['model_id'])) {
            ...
            if (!$query) {
                ...
                if (!empty($_SESSION['registreren']['keywords'])) {
                    ...
                    $model = $this->entityFactory->model()->findOneById($model_id);
    ...
    public function ajax_foto_upload()

    {
        ...
        if (isset($_GET['qqfile'])) {
            ...
            if (isset($_SESSION['registreren']['fotos'])) {

                $next = count($_SESSION['registreren']['fotos']) + 1;

                $code = $_SESSION['registreren']['fotos_code'];
            ...
            include "php/core/lib/upload.php";

            include "php/core/lib/qqupload.php";
            ...
            $upload = new qqFileUploader($extentions, $limiet);

            $result = $upload->handleUpload($dir);



            if (isset($result['success']) == 1) {

                $upload = new Upload($dir . $file);



                if ($upload->uploaded) {

                    $upload->file_new_name_body = $next . '_thumb';
...
$remote_file = '/www/testimages/images/'.$file;

						$file_upload = $dir . $file;
                    ...
                    if (!$upload->processed || !file_exists($dir . $file)) {

                        $return['bericht'] = ("Er is een technisch probleem opgetreden tijdens het uploaden van de foto: " . $upload->error);
                        ...
                        $afmetingen = getimagesize($dir . $file);

                        $breedte = $afmetingen[1];

                        $upload->file_new_name_body = $next;
                        ...
                        if (!$upload->processed) {
                            ...
                            $return['url'] = $dir . $next . "_thumb.jpg";

                            $return['success'] = 1;



                            $_SESSION['registreren']['fotos_code'] = $code;

                            $_SESSION['registreren']['fotos'][$next] = 1;
...

    {
        ...
        if (isset($_POST['code'])) {
            ...
            if ($check->num_rows > 0) {
                ...
                $modelEntity = $this->entityFactory->model()->findOneById($model['model_id']);

                /** @var Model $modelEntity */



                if ($modelEntity && count($modelEntity->getKeywords())) {

                    $keywords = '';



                    foreach ($modelEntity->getKeywords() as $keyword) {
                    ...
                    $_SESSION['registreren']['keywords'] = trim($keywords);
